...
const SubscriptionPlans = ({ hideSidebar = false }) => {
    ...
    const handlePay = async (plan, months = 1) => {
        ...
        try {
            ...
            showValidationError(error, 'Không thể gửi yêu cầu đăng ký gói');
        } finally {
    ...
    const handleExtendSubscription = async () => {
        ...
        try {
            ...
            showValidationError(error, 'Không thể gửi yêu cầu gia hạn');
        } finally {
            setPayingPlanId(null);
        }
    };
    ...
    const confirmChangePlan = async () => {
        ...
        try {
            ...
            showValidationError(error, 'Không thể gửi yêu cầu đổi gói');
        }
    };
    ...
    const requestOfflineInvoicePayment = async (invoiceId, paymentMethod) => {
        ...
        try {
            ...
            showValidationError(error, 'Không thể gửi yêu cầu thanh toán');
        } finally {
    ...
    const payInvoiceByVnPay = async (invoice) => {
        ...
        try {
            ...
            if (!paymentUrl) {
                showValidationError('Không thể tạo link thanh toán VNPay');
                return;
            }
            window.location.href = paymentUrl;
        } catch (error) {
            showValidationError(error, 'Không thể tạo thanh toán VNPay');
        } finally {
            setSubmittingInvoicePayment(false);
        }
    };
